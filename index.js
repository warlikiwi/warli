...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `tchau @${num.split('@')[0]}foi tarde seu bosta👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ aguarde o processo seu bosta⌛',
				success: '✔️pronto vadia✔️',
				error: {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado quando o bot se torna administrador! ❌',
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna administrador! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["64999747515@s.whatsapp.net"] // substitua isso pelo seu número
			...
			switch(command) {
				...
				case 'bem vindo':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('bem vido')
					if (Number(args[0]) === 1) {
